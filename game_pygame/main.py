...
# Получаем размер экранаimport pygame
import sys
...
class Grid:
    ...
    def step(self):
        ...
        # камера вправо
        if (mouse_pos[0] >= self.screen_width - 2 or keys[pygame.K_RIGHT]) and self.pos_x >= self.screen_width - 2500:
        ...
        # камера вниз
        if (mouse_pos[1] >= self.screen_height - 2 or keys[pygame.K_DOWN]) and self.pos_y >= self.screen_height - 2000:
        ...
        # камера влево
        if (mouse_pos[0] <= 2 or keys[pygame.K_LEFT]) and self.pos_x <= self.screen_width - 2000:
            self.pos_x += self.step_x
            for rect_info in self.rectangles:
                rect = pygame.Rect(rect_info['x'] + self.step_x, rect_info['y'], self.width, self.height)
                rect_info['rect'] = rect
